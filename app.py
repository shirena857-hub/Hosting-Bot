...
BOT_TOKEN = "8642055013:AAGFs4ti8RVhOZ8RP2Ta8xrXzSsT6b4dwsY"
OWNER_ID_STR = "6804172454"
ADMIN_ID_STR = "6804172454"
OWNER_CONTACT_URL = "https://t.me/shihab_ff_857"
UPDATE_CHANNEL_URL = "https://t.me/shihab_ff_66bot"
...
if not OWNER_ID_STR or not ADMIN_ID_STR:
    logger.error("OWNER_ID or ADMIN_ID is missing!")
...
# Project Limits
FREE_TIER_SLOTS = 1
PREMIUM_TIER_SLOTS = 10      # মোট কতগুলো প্রিমিয়াম স্লট বিক্রি করা যাবে
SLOT_PRICE_STARS = 5
...
class AdminFSM(StatesGroup):
    ...
    awaiting_premium_slots = State()
    awaiting_remove_premium_id = State()
...
async def init_db():
    ...
    try:
        ...
        await c.execute('''CREATE TABLE IF NOT EXISTS premium_subscriptions (
...
async def get_total_purchased_slots() -> int:
    conn = await get_db_conn()
    c = await conn.cursor()
    await c.execute("SELECT SUM(purchased_slots) FROM users")
    result = await c.fetchone()
    await conn.close()
    return result[0] if result[0] is not None else 0

async def get_user_total_premium_slots(user_id: int) -> int:
    conn = await get_db_conn()
    c = await conn.cursor()
    await c.execute("SELECT purchased_slots FROM users WHERE user_id = ?", (user_id,))
    row = await c.fetchone()
    permanent = row[0] if row and row[0] is not None else 0
    now = datetime.now().isoformat()
    await c.execute("SELECT SUM(slots) FROM premium_subscriptions WHERE user_id = ? AND expiry_date > ?", (user_id, now))
    row = await c.fetchone()
...
async def get_user_slot_limit(user_id: int) -> int:
    if user_id in admin_ids:
        return float('inf')
    try:
        premium = await get_user_total_premium_slots(user_id)
        if premium is None:
            premium = 0
        return FREE_TIER_SLOTS + premium
    except Exception as e:
        logger.error(f"Error in get_user_slot_limit for {user_id}: {e}")
        return FREE_TIER_SLOTS
...
⭐ <b>Premium Tier:</b> Purchase additional slots.
...
async def cmd_new_project(message: types.Message, state: FSMContext):
    ...
    # ensure integer
    if slot_limit is None or not isinstance(slot_limit, (int, float)):
        slot_limit = FREE_TIER_SLOTS

    projects = await get_user_projects(user_id)
    if len(projects) >= slot_limit:
        await message.answer(
            f"❌ <b>Project Limit Reached!</b>\n\n"
            f"Your current limit is <b>{slot_limit}</b> project(s).\n\n"
            f"Please delete an existing project or buy a new slot from the 'My Quota' menu."
        )
...
async def fsm_project_name(message: types.Message, state: FSMContext):
    ...
    # Double-check limit (race condition safety)
    slot_limit = await get_user_slot_limit(user_id)
    if slot_limit is None or not isinstance(slot_limit, (int, float)):
        slot_limit = FREE_TIER_SLOTS
    projects = await get_user_projects(user_id)
    if len(projects) >= slot_limit:
        await message.answer(
            f"❌ <b>Project limit reached!</b>\n"
            f"You already have {len(projects)} / {slot_limit} projects.\n"
            f"Please delete an existing project or buy more slots."
        )
        await state.clear()
        await callback_my_projects(message)
        return
...
async def callback_my_projects(callback_or_message: types.CallbackQuery | types.Message):
    ...
    if not projects:
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="🔙 Back to Main", callback_data="main_menu_from_projects")]
        ])
    else:
        ...
        buttons.append([InlineKeyboardButton(text="🔙 Back to Main", callback_data="main_menu_from_projects")])
...
@dp.callback_query(F.data == "main_menu_from_projects")
async def callback_main_menu_from_projects(callback: types.CallbackQuery, state: FSMContext):
    await state.clear()
...
async def callback_view_quota(callback: types.CallbackQuery):
    ...
    premium_slots = await get_user_total_premium_slots(user_id)
    total_purchased = await get_total_purchased_slots()
    text = """
📊 <b>Your Quota</b>
...
Slots Used: {}
Free Slots: {} / {}
Premium Slots (active): {}

<b>Premium Tier</b>
Available Premium Slots (global): {} / {}
<i>Purchase slots for monthly 5 Stars each.</i>
RAM per project: 1024MB

Click below to buy more slots.
""".format(tier_name, len(projects), FREE_TIER_SLOTS, FREE_TIER_SLOTS, premium_slots, total_purchased, PREMIUM_TIER_SLOTS)
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🏠 Main Menu", callback_data="main_menu_from_quota")]
    ])
    await callback.message.edit_text(text, reply_markup=keyboard)
    await callback.answer()

@dp.callback_query(F.data == "main_menu_from_quota")
async def callback_main_menu_from_quota(callback: types.CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.message.delete()
    await cmd_start(callback.message, state)
    await callback.answer()
...
async def callback_buy_slot(callback: types.CallbackQuery):
    total_purchased = await get_total_purchased_slots()
    if total_purchased >= PREMIUM_TIER_SLOTS:
        await callback.answer("❌ All premium slots are sold out! Contact admin.", show_alert=True)
        return
...
async def handler_successful_payment(message: types.Message):
    ...
    if payload.startswith("buy_slot_"):
        total_purchased = await get_total_purchased_slots()
        if total_purchased >= PREMIUM_TIER_SLOTS:
            await message.answer("❌ Sorry, all premium slots were sold out while processing your payment. Please contact admin for a refund.")
            await bot.send_message(OWNER_ID, f"🚨 Payment received but slots full! User {user_id} paid but cannot add slot.")
            return
        try:
            conn = await get_db_conn()
            c = await conn.cursor()
...
""".format(project_name, status_icon, status.capitalize(), '512MB (Free)' if ram == 'free' else '1024MB (Premium)', run_cmd)
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🔙 Back to Projects", callback_data="my_projects")],
        [InlineKeyboardButton(text="🏠 Main Menu", callback_data="main_menu_from_manage")]
    ])
    if edit:
        try:
            await message.edit_text(text, reply_markup=keyboard)
...
@dp.callback_query(F.data == "main_menu_from_manage")
async def callback_main_menu_from_manage(callback: types.CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.message.delete()
    await cmd_start(callback.message, state)
    await callback.answer()
...
async def callback_delete_project(callback: types.CallbackQuery, state: FSMContext):
    project_name = callback.data.split(":", 1)[1]
    user_id = callback.from_user.id
    await state.update_data(project_name=project_name, user_id=user_id)
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Yes, Delete", callback_data=f"confirm_delete:{project_name}")],
        [InlineKeyboardButton(text="❌ No, Cancel", callback_data=f"cancel_delete:{project_name}")]
    ])
    await callback.message.edit_text(
        f"⚠️ <b>Are you sure you want to delete {project_name}?</b>\n\nThis action cannot be undone.",
...
async def callback_confirm_delete(callback: types.CallbackQuery, state: FSMContext):
    project_name = callback.data.split(":", 1)[1]
    user_id = callback.from_user.id
    await callback.message.edit_text(f"Deleting <b>{project_name}</b>...")
    try:
        await stop_project(user_id, project_name, callback.message)
        await delete_project_from_db(user_id, project_name)
        ...
        await callback.message.answer(f"✅ Project <b>{project_name}</b> has been deleted.")
    except Exception as e:
        logger.error(f"Delete error: {e}")
        await callback.message.answer(f"❌ Error: {e}")
    finally:
        await state.clear()
        await callback_my_projects(callback.message)

@dp.callback_query(F.data.startswith("cancel_delete:"))
async def callback_cancel_delete(callback: types.CallbackQuery, state: FSMContext):
    project_name = callback.data.split(":", 1)[1]
    user_id = callback.from_user.id
    await state.clear()
    await send_deployment_menu(callback.message, user_id, project_name, edit=True)
...
def get_admin_panel_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            InlineKeyboardButton(text="📋 Premium List", callback_data="admin_premium_list"),
...
async def callback_admin_user_stats(callback: types.CallbackQuery):
    ...
    await c.execute("SELECT SUM(purchased_slots) FROM users")
    total_purchased = (await c.fetchone())[0] or 0
    await conn.close()
    text = """
👥 <b>User Statistics</b>
...
🚫 <b>Banned Users:</b> {}
✅ <b>Active Users:</b> {}
⭐ <b>Premium Slots (permanent):</b> {} / {}
""".format(total_users, banned_count, total_users - banned_count, total_purchased, PREMIUM_TIER_SLOTS)
...
async def callback_admin_backup_db(callback: types.CallbackQuery):
    ...
    try:
        ...
        conn = await get_db_conn()
        backup_conn = await sqlite3.connect(backup_path)
...
async def callback_admin_premium_list(callback: types.CallbackQuery):
    if callback.from_user.id not in admin_ids:
        return await callback.answer("❌ Admin only!")
    conn = await get_db_conn()
    conn.row_factory = sqlite3.Row
    c = await conn.cursor()
    now = datetime.now().isoformat()
    await c.execute("SELECT user_id, slots, expiry_date FROM premium_subscriptions WHERE expiry_date > ? ORDER BY expiry_date", (now,))
    subs = await c.fetchall()
    await conn.close()
    if not subs:
        text = "📋 No active premium subscriptions."
    else:
        text = "📋 <b>Active Premium Subscriptions</b>\n\n"
        for s in subs:
            text += f"👤 <code>{s['user_id']}</code> | 🎫 {s['slots']} slots | ⏳ expires: {s['expiry_date']}\n"
    await callback.message.edit_text(text, reply_markup=get_admin_panel_keyboard())
    await callback.answer()

# --- Add Premium FSM ---
@dp.callback_query(F.data == "admin_add_premium")
async def callback_admin_add_premium_start(callback: types.CallbackQuery, state: FSMContext):
    if callback.from_user.id not in admin_ids:
        return await callback.answer("❌ Admin only!")
    await state.set_state(AdminFSM.awaiting_premium_user_id)
    await callback.message.edit_text(
        "Enter User ID to add premium:\n\nSend /cancel to abort.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🚫 Cancel", callback_data="fsm_cancel")]])
    )

@dp.message(AdminFSM.awaiting_premium_user_id, F.text, ~F.text.startswith('/'))
async def fsm_premium_user_id(message: types.Message, state: FSMContext):
    try:
        user_id = int(message.text)
        await state.update_data(premium_user_id=user_id)
        await state.set_state(AdminFSM.awaiting_premium_days)
        await message.answer("Enter number of days until expiry (e.g., 30):")
    except ValueError:
        await message.answer("❌ Invalid ID. Enter numbers only.")

@dp.message(AdminFSM.awaiting_premium_days, F.text, ~F.text.startswith('/'))
async def fsm_premium_days(message: types.Message, state: FSMContext):
    try:
        days = int(message.text)
        if days <= 0:
            raise ValueError
        await state.update_data(premium_days=days)
        await state.set_state(AdminFSM.awaiting_premium_slots)
        await message.answer("Enter number of slots to grant:")
    except ValueError:
        await message.answer("❌ Invalid number. Enter positive integer.")

@dp.message(AdminFSM.awaiting_premium_slots, F.text, ~F.text.startswith('/'))
async def fsm_premium_slots(message: types.Message, state: FSMContext):
    try:
        ...
        days = data['premium_days']
        expiry = (datetime.now() + timedelta(days=days)).isoformat()
        now = datetime.now().isoformat()
        conn = await get_db_conn()
        c = await conn.cursor()
        await c.execute('INSERT INTO premium_subscriptions (user_id, slots, expiry_date, created_at) VALUES (?, ?, ?, ?)',
                        (user_id, slots, expiry, now))
        await conn.commit()
        await conn.close()
        await message.answer(f"✅ Added {slots} premium slot(s) to user {user_id} until {expiry}.")
        await state.clear()
        await callback_my_projects(message)
    except ValueError:
        await message.answer("❌ Invalid slots. Enter positive integer.")

# --- Remove Premium FSM ---
@dp.callback_query(F.data == "admin_remove_premium")
async def callback_admin_remove_premium_start(callback: types.CallbackQuery, state: FSMContext):
    if callback.from_user.id not in admin_ids:
        return await callback.answer("❌ Admin only!")
    await state.set_state(AdminFSM.awaiting_remove_premium_id)
    await callback.message.edit_text(
        "Enter User ID to remove premium slots:\n\nSend /cancel to abort.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🚫 Cancel", callback_data="fsm_cancel")]])
    )

@dp.message(AdminFSM.awaiting_remove_premium_id, F.text, ~F.text.startswith('/'))
async def fsm_remove_premium(message: types.Message, state: FSMContext):
    try:
        ...
        c = await conn.cursor()
        now = datetime.now().isoformat()
        await c.execute("SELECT id, slots FROM premium_subscriptions WHERE user_id = ? AND expiry_date > ?", (user_id, now))
        subs = await c.fetchall()
        if not subs:
            await message.answer("❌ No active premium subscriptions found for this user.")
            await conn.close()
            await state.clear()
            await callback_my_projects(message)
            return
        sub_id = subs[0][0]
        await c.execute("DELETE FROM premium_subscriptions WHERE id = ?", (sub_id,))
        await conn.commit()
        await conn.close()
        await message.answer(f"✅ Removed 1 premium subscription from user {user_id}.")
        await state.clear()
        await callback_my_projects(message)
    except ValueError:
